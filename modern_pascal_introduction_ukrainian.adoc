...
У світі існує безліч книг про Pascal, однак, багато з них говорять про "застарілий" Pascal, без класів, модулів, дженериків.
...
Тож я написав цей короткий вступ до того, що я називаю *сучасний Object Pascal*. Більшість програмістів, які його використовують, насправді не називають його _"сучасний Object Pascal"_, ми просто називаємо його _"наш Pascal"_. Але, представляючи мову, я вважаю важливим підкреслити, що це сучасна, об’єктно-орієнтована мова. Він *багато* еволюціонував після старого (Turbo) Pascal, який багато людей вивчали колись в школах. За функціями він дуже схожий на C++, Java або C#.

* Pascal має всі сучасні особливості, які можна очікувати - класи, модульну структуру, інтерфейси, дженеріки...
* Pascal компілюється в швидкий машинний код.
* Pascal є типобезпечною мовою, що в деяких випадках суттєво спрощує налагодження.
* В основному Pascal - високорівнева мова, проте він має можливість використовувати низькорівневі підходи, якщо це необхідно.

Він також має чудовий кросплатформовий компілятор з відкритим вихідним кодом: _Free Pascal Compiler_ (http://freepascal.org/). Для нього існує декілька IDE (включаючих в себе редактор, відлагоджувач, бібліотеки компонентів, дизайнер форм), одна з найвідоміших називається _Lazarus_ (http://lazarus.freepascal.org/). Автор цієї книги є творцем _Castle Game Engine_ (https://castle-engine.io/) - котрий є потужним дво- і тримірним ігровим рушієм, що використовує цю мову, для створення ігор під багато платформ: Windows, Linux, MacOSX, Android, iOS, Nintendo Switch; також очікується WebGL.
...
_Прим.перекл. Тут і далі ми будемо використовувати оригінальні англомовні терміни, такі як unit, constructor, override, reference-counting у випадках, якщо вони є ключовими словами мови або не мають загальноприйнятих перекладів на українську мову. Також часто надається перевага використанню оригінального англійського слова його транслітерації, як у випадку з поняттям "generic" вище._
...
[source,pascal]
----
include::code-samples_ukrainian/hello_world.dpr[]
----

Це повноцінна програма, яку можна _компілювати_ та _запускати_.

* Якщо Ви користуєтесь компілятором FPC з командного рядка, просто створіть новий файл `myprogram.dpr` з кодом програми всередині й виконайте команду `fpc myprogram.dpr`.
...
[source,pascal]
----
include::code-samples_ukrainian/functions_primitives.dpr[]
----
...
Використовується _оцінка короткого замикання_. Розглянемо такий вираз:
...
----
+
Це буде нормально, навіть якщо `A` дорівнює `nil`. Ключове слово `nil` — це вказівник, що дорівнює нулю (коли представлено у вигляді числа). Це називається _нульовим покажчиком_ у багатьох інших мовах програмування.
...
Якщо ви походите з C-подібних мов, і порівняйте це з інструкцією `switch` у цих мовах, ви помітите, що немає автоматичного _провалу_. Це свідоме благословення в Паскалі. Вам не потрібно пам’ятати про розміщення інструкцій `break`. Під час кожного виконання виконується _щонайбільше одна_ гілка `case`, і все.

### Перелічувані та порядкові типи, множини та масиви постійної довжини
...
NOTE: Зіткнення в іменах не перешкода. Це нормально, якщо різні модулі визначають той самий ідентифікатор. Але було б гарною ідеєю все одно намагатися уникнути колізій, щоб код був простим для розуміння та пошуку.

NOTE: Ви можете уникнути розміщення імен enum у глобальному просторі імен за допомогою директиви компілятора `{$scopedenums on}`. Це означає, що вам доведеться отримати до них доступ, визначивши назву типу, як-от `TAnimalKind.akDuck`. У цій ситуації потреба в префіксі `ak` зникає, і ви, ймовірно, просто називатимете переліки `Duck, Cat, Dog`. Це схоже на перерахування C#.

Той факт, що перерахований тип є _непрозорим_, означає, що його не можна просто призначити цілому числу. Однак для спеціального використання ви можете використати `Ord(MyAnimalKind)` для примусового перетворення enum на int або приведення типу `TAnimalKind(MyInteger)` для примусового перетворення int на enum. В останньому випадку переконайтеся, що спершу перевірте, чи `MyInteger` знаходиться в належному діапазоні (від 0 до `Ord(High(TAnimalKind))`).
...
[source,pascal]
----
include::code-samples_ukrainian/loops.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/for_in_list.dpr[]
----
+
...
[source,pascal]
----
include::code-samples_ukrainian/myunit_test.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/inheritance.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/is_as.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/exception_finally.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/method_calls_inheritance_1.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/method_calls_inherited.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/without_virtual_methods.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/with_virtual_methods.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/file_stream.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_lists.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_sorting.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_dictionary.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generics_object_dictionary.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/persistent.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/callbacks.dpr[]
----
* A method: declare with `of object` at the end.
+
[source,pascal]
----
include::code-samples_ukrainian/callbacks_of_object.dpr[]
----
+
...
[source,pascal]
----
include::code-samples_ukrainian/generics.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/generic_functions.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/records.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_classes.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_records.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/operator_overloading_records_lists.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/static_class_method.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/class_properties.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/exception_in_constructor_test.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_corba_test.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/gen_guid.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_com_with_ref_counting.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interfaces_com_test.dpr[]
----
...
[source,pascal]
----
include::code-samples_ukrainian/interface_casting.dpr[]
----
